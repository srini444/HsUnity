...
public class Server_Vehicle_TrafficHandler : NetworkBehaviour {
	...
	void FixedUpdate () {
		if (isServer) { //Call these function only if we are the host client
			Sensors ();
			VehicleMove ();
			CheckForWayPoints ();
		}
	...
	void GetNextNode() //Get the next node to move to
	{
		...
		CurrentNode = CurrentNode.GetComponent<Traffic_Node> ().NextNodes [Random.Range(0,CurrentNode.GetComponent<Traffic_Node> ().NextNodes.Count)]; //Get the next node
	}
